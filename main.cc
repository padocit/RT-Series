...
int main() {
    ...
    auto R = std::cos(kPi / 4); // radius

    auto matLeft = make_shared<Lambertian>(Color(0, 0, 1));
    auto matRight = make_shared<Lambertian>(Color(1, 0, 0));

    world.Add(make_shared<Sphere>(Point3(-R, 0, -1), R, matLeft));
    world.Add(make_shared<Sphere>(Point3(R, 0, -1), R, matRight));
